...
#@Time      : 2021/12/26  21:10
#@Author    : An
#@File      : main_multi_threading_5173_trades.py
#@Software  : PyCharm
...
class TradeDd373Thread(threading.Thread):
    ...
    def game_account_page(self,start_url):
        ...
        self.browser.get\
            (start_url)
        goods_select_value=self.browser.find_element_by_css_selector("[class='list-panel select-type']")
    ...
    def check_next_page(self):
        ...
        while True:
            try:
                laypage_cur = int(self.browser.find_element_by_xpath("//input[@id='_Pager_Ctrl0_pib']").get_attribute("value"))
        ...
        page_num_tag_list = laypage_main.find_elements_by_xpath('./a')
        ...
        if '下一页' in page_name_list:
            ...
            while check_times< 3 :
                # 当前第N页
                next_page_num = int(self.browser.find_element_by_xpath("//input[@id='_Pager_Ctrl0_pib']").get_attribute("value"))
...
class ParseTradeDd373Thread(threading.Thread):
    ...
    def parse_next_page_detail(self,html):
        ...
        for item_box in goods_list_item:
            ...
            try:
                text_center = item_box.xpath(".//td[@class='text-center']/a/@onclick")[0].replace("gioclick","")[1:-1].split(",")

                goods_code =text_center[0]
                goods_title =text_center[1]
                goods_type =text_center[2]
                goods_price =text_center[3]
                goods_game =text_center[4]
                goods_os =text_center[5]
            except:
                goods_code =''
                goods_title =''
                goods_type =''
...
def main():
    ...
    for game in browser.find_elements_by_xpath("//ul[contains(@class,'mobilegame-hotspell')]/li/a[@class='link-all']"):
    ...
    trade_thread_name_list = ['trade' + str(i) for i in range(3)]
    trade_thread_list = []
    ...
    parse_thread_name_list = ['parse' + str(i) for i in range(3)]
    parse_thread_list = []
