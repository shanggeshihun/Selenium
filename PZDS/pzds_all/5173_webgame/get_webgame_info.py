...
def webgame_info():
    ...
    # webgame webgame-hotspell
    webgame_bnt = browser.find_element_by_xpath("//li[@data-tab='webgame']")
    webgame_bnt.click()
    time.sleep(2)


    # 游戏基本信息保存到文件
    f = open("./webgame_info.txt","w",encoding="utf-8")
    ...
    for fl in first_letter:
        ...
        game_xpath_name = "//ul[@class='gamelist-ul clearfix webgame-{}']/li/a[@class='link-all']".format(game_first_letter)
...
def test_read():
    f = open("./webgame_info.txt", "r", encoding="utf-8")
    lines = f.readlines()
