...
class TradeDd373Thread(threading.Thread):
    ...
    def game_account_page(self, start_url):
        ...
        """
        网站有反爬机制，当点击到一定数量会提示"阻隔页面"
        """
        if self.browser.page_source.find("由于您访问的URL") != -1:
            warning_email()
            warning_email()
            time.sleep(300)
        else:
    ...
    def check_next_page(self):
        ...
        while try_times < 3:
            try:
                ...
                print('【异常】手工处理:', e, self.browser.current_url)
                try_times += 1
                time.sleep(2)
                self.browser.refresh()
            else:
...
class ParseTradeDd373Thread(threading.Thread):
        ...
        for item_box in goods_list_item:
            ...
            try:
                text_center = item_box.xpath(
                    "./ul[@class='pdlist_way']/li[@class='goto_buy']/a/@onclick")[0].replace("gioclick", "")[
                              1:-2].split(",")
            ...
            # print(item)
            filed_tuple = (
                'game_page_url','game_name','zone_name','title','price','marchant_credit','marchant_credit_icon','publish_time','ensure','goods_code','goods_title','goods_type','goods_price','goods_name','goods_os')
    ...
    def insert_db(self, filed_tuple, filed_values_tuple):
        try:
            sql = "insert into trade_5173_webgame_result({})values{}".format(','.join(filed_tuple), filed_values_tuple)
...
def main():
    ...
    with open("./webgame_info.txt", "r", encoding="utf-8") as f:
        lines = f.readlines()
