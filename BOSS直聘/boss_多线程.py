...
class Get_xinxi(threading.Thread):
    ...
    def run(self):
        ...
        # 最终装饰成浏览器可以去发送解析url
        drive_path = r"C:\Users\Administrator\AppData\Local\Google\Chrome\Application\chromedriver.exe"
        browser = webdriver.Chrome(options=chromeOptions,executable_path=drive_path)
        while True:
            xinxi = queue.get()
            browser.get(xinxi[4])
...
if __name__ == '__main__':
    df = pd.read_csv('./data/Bossaa.csv', sep=',', encoding='utf-8')
